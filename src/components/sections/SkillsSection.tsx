...
export default function SkillsSection({ locale }: SkillsSectionProps) {
  ...
  return (
    <section id="competencias" aria-labelledby="competencias-title" className="border-t border-[var(--divider)] py-[72px] md:py-[88px]">
      <Container>
        <header className="mb-8 md:mb-10">
          <h2 id="competencias-title" className="text-[30px] font-semibold leading-[1.1] tracking-[-0.035em] text-[var(--text)] md:text-[34px]">{copy.competencies.title}</h2>
        </header>
        <div>
          {copy.competencies.items.map((group, index) => (
            <DisclosureItem
              key={group.title}
              index={String(index + 1).padStart(2, "0")}
              title={group.title}
              summary={group.terms.join(" · ")}
              className={index === copy.competencies.items.length - 1 ? "border-b-0" : "border-b border-[var(--divider-item)]"}
            ...
            >
              <p className="max-w-[720px] text-[14px] leading-[1.7] text-[var(--muted)]">{group.detail}</p>
            </DisclosureItem>
          ))}
        </div>
      </Container>
